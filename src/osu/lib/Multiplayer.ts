...
import nodesuClient from './nodesu'
import client, { logger } from '..'
import config from '../../config'
import formattedCommandList from '../commands'
import * as ENUM from './enum'
import { CommandExecutorRole, CommandWhereExecutied } from './Commands'
  ...
  constructor(
    ...
    this.lobby.on('playerLeft', async (obj) => {
      if (this.master?.id == obj.user.id) {
        if (this.multiplayerChannel.channelMembers.size > 0) {
          this.multiplayerChannel.channelMembers.forEach((value) => {
            this.setMaster = value.user
            return false
          })
        } else {
          this.master = undefined
